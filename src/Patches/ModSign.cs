...
public class ModSign : ModCustom
{
    ...
    {
        return "WorldObjects/Other/Sign";
    }
    ...
    public void CreateSign(string UniqueName, int Level, int Range, string[] NewIngredientsStringArr = null, int[] NewIngredientsAmountArr = null, string ModelName = "", bool UsingCustomModel = true)
    {
        ...
        if (!GeneralUtils.m_InGame)
        {
            ...
            if (DebugInfo)
            {
                Debug.Log("ADDED NEW SIGN CALLED " + UniqueName + " (" + UniqueName + ")  ObjID " + objectType);
            }
